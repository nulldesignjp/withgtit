...
void main()
{
    ...
    if (vBlurAmount > 0.1) {
        ...
        // 外側のエッジ
        float rad = smoothstep(outerRadius + ringThickness, outerRadius, dist); 
        float outerEdge = sin( rad * 3.14 - 1.57 ) * 0.5 + 0.5;
        
        // 内側のエッジ
        float innerEdge = 0.0;
        
        // リング形状を作成
        opacity = outerEdge + innerEdge;
        
        // ボケ量に応じてリングを明るくする
        opacity *= mix(0.0, 1.0, vBlurAmount);
        
    } else {
        // 焦点内: 通常の円形
        opacity = smoothstep(1.0, 0.0, dist);
    }
    ...
    init() {
        ...
        for (var i = 0; i < this.WIDTH; i++) {
            for (var j = 0; j < this.WIDTH; j++) {
                ...
                positions[_i * 3 + 2] =
                    Math.cos((i - this.WIDTH / 2) / this.WIDTH * Math.PI * 4) * 32.0 +
                    Math.sin((j - this.WIDTH / 2) / this.WIDTH * Math.PI * 4) * 32.0;
